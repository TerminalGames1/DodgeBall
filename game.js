...
btnPause.addEventListener("click", () => {
  ensureAudioUnlocked();
  // If we're on the start screen, treat Pause as Play (feels intuitive)
...
// ✅ Start menu play “button” area (still allows tap-anywhere)
function uiPlayRect() {
  const w = 190, h = 52;
...
document.addEventListener("keydown", (e) => {
  ...
  // ✅ Start menu: any non-modifier key starts
  if (Game.running && !Game.started) {
    const k = e.key;
...
// Pointer: start menu tap-to-play; tap to cycle lane; tap-to-resume; tap-to-restart.
...
canvas.addEventListener("pointerdown", async (e) => {
  ...
  // ✅ Start menu: tap to play (tap-anywhere works; play button is just visual)
  if (Game.running && !Game.started) {
    const pr = uiPlayRect();
    // If they tap play area (or anywhere), start
    if (inRect(x, y, pr) || true) {
      startGameNow();
      playClick();
      return;
    }
  ...
  // Canvas pause button always active if game is running + started
  if (Game.running && Game.started) {
...
// -------- Update + Draw --------
function update(dt) {
  // ✅ start menu animation timer (even while paused)
...
function draw() {
  ...
  // Canvas Pause button (always visible while running + started)
  if (Game.running && Game.started) {
  ...
  // ✅ START MENU (Ball Blazter) — glow + pulse
  if (Game.running && !Game.started) {
